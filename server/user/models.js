...
const userSchema = mongoose.Schema(
  {
    ...
    avatarUrl: {
      type: String,
      required: true,
      default: "/static/images/ic_tag_faces_white_18px.svg"
    },
...
userSchema.index({
  optIntoLeaderboard: 1,
  bannedFromLeaderboard: 1,
  score: -1
});
  ...
  return this.find().then(users => {
    for (const user of users) {
      ...
      for (const question of questions) {
        ...
        const userAnswer = user.answers.find(answer =>
          question._id.equals(answer.questionId)
        );
